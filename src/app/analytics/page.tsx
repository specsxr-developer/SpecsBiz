...
export default function AnalyticsPage() {
  ...
  const chartData = useMemo(() => {
    ...
    if (timeRange === "week") steps = eachDayOfInterval({ start: startOfWeek(now), end: endOfWeek(now) })
      ...
      return { name: format(m, "MMM"), revenue: monthSales.reduce((sum, s) => sum + (s.total || 0), 0), profit: monthSales.reduce((sum, s) => sum + (sum.profit || 0), 0) }
    })
  ...
  const finalSalesList = filteredSales.filter(s => (s.items?.map((i: any) => i.name).join(' ') || "").toLowerCase().includes(search.toLowerCase()))
